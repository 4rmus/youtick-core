...
var LitClient = class {
  ...
  async getSessionSigsWithPKP(pkpPublicKey, pkpEthAddress, nearAccountId, capacityDelegationAuthSig) {
    ...
    const sessionParams = {
      ...
      resourceAbilityRequests: [
        {
          resource: new import_auth_helpers.LitAccessControlConditionResource("*"),
          ability: import_constants.LitAbility.AccessControlConditionDecryption
        },
        {
          resource: new import_auth_helpers.LitPKPResource("*"),
          ability: import_constants.LitAbility.PKPSigning
        },
        {
          resource: new import_auth_helpers.LitActionResource("*"),
          ability: import_constants.LitAbility.LitActionExecution
        }
      ],
      expiration: new Date(Date.now() + 1e3 * 60 * 60 * 24).toISOString()
      // 24 hours
...
function deriveChildKey(masterKeyStr, accountId, path) {
  ...
  const scalarHex = (0, import_js_sha3.sha3_256)(derivation_path);
  const scalar = new import_bn.default(scalarHex, 16);
